...
export interface TerminologyTreeviewSelection {
  checkedItems: TerminologyTreeviewItem[];
  uncheckedItems: TerminologyTreeviewItem[];
}
...
export class TerminologyTreeviewItem {
  ...
  constructor(item: TerminologyTreeItem, autoCorrectChecked = false) {
    ...
    if (!isNil(item.children) && item.children.length > 0) {
      this.children = item.children.map(child => {
        ...
        return new TerminologyTreeviewItem(child);
      });
    }
  ...
  get children(): TerminologyTreeviewItem[] {
    return this.internalChildren;
  }

  set children(value: TerminologyTreeviewItem[]) {
  ...
  getSelection(): TerminologyTreeviewSelection {
